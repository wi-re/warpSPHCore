...
def computeSPHDivergenceTensor_Func_i(
    ...
    for neighborIndex in range(numIndices):
        ...
        # Explicit if/else, not a ternary: see this file's own useGradHTerms comment a few
        # lines down -- confirmed (2026-08-21, `warpier_tier2_correction_jvp_plan.md` phase
        # b's spike) that this exact "different arrays per branch" ternary DOES silently
        # zero d(output)/d(referenceVolumes) under the installed warp 1.16.0, contradicting
        # `docs/lessons_learned.md`'s earlier "confirmed non-issue" note for this pattern --
        # see that doc's updated entry.
        if correctionData.useVolume:
            if consistentDivergence:
                apparentVolume = correctionData.referenceVolumes[j] * jPtcl.density / iPtcl.density
            else:
                apparentVolume = correctionData.referenceVolumes[j]
        else:
            if consistentDivergence:
                apparentVolume = jPtcl.mass / iPtcl.density
            else:
