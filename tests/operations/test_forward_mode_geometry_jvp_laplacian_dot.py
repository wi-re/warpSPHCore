...
def test_laplacianDotGeometryJVP_accepts_crkState_and_renormalizationState():
    ...
    n = positions.shape[0]
    crkDummy = CRKState(A=torch.zeros(n, dtype=DTYPE), B=torch.zeros(n, 2, dtype=DTYPE),
                        gradA=torch.zeros(n, 2, dtype=DTYPE), gradB=torch.zeros(n, 2, 2, dtype=DTYPE))
    renormDummy = RenormalizationState(renormalizationMatrices=torch.zeros(n, 2, 2, dtype=DTYPE))
