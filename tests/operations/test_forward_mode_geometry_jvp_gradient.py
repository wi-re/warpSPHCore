"""In-process standing test for `warpOperationJVP`'s geometry JVP Gradient branch
(`warpier_tier2_operators_plan.md` Step 4, `warpier_adjoint.md` Tier 2.2):
asserts `computeSPHGradientGeometryJVP` matches a reverse-mode-Jacobian
reference on the production `warpOperation(Gradient)` call, the same
reference-construction pattern `test_forward_mode_geometry_jvp_density.py`/
`test_forward_mode_geometry_jvp_interpolate.py`/`spike_forward_mode_tier2_gradient.py`
use.

`queryValues`/`referenceValues` (`fi`/`fj`) are frozen throughout the
geometry-JVP-only cases below -- only positions/supports/masses/densities are
differentiated. The "combined" case exercises the sum of this geometry JVP
contribution with the value-tangent (value JVP) contribution
...
def _check_jacobian_reference(positions, supports, masses, domain, adjacency, mode, scheme, valueShapeSuffix=()):
    n = positions.shape[0]
    ...
    queryValues = torch.randn(n, *valueShapeSuffix, dtype=DTYPE, device=DEVICE)
    referenceValues = torch.randn(n, *valueShapeSuffix, dtype=DTYPE, device=DEVICE)
...
def _check_combined_jacobian_reference(positions, supports, masses, domain, adjacency, mode, scheme, valueShapeSuffix=()):
    ...
    # verified against a jacobian differentiating w.r.t. every input
    # (positions/supports/masses/densities/queryValues/referenceValues) at
    # once, not just geometry as `_check_jacobian_reference` above does.
    n = positions.shape[0]
    kinds = torch.zeros(n, dtype=torch.int32, device=DEVICE)
    densities = _densities_for(positions, supports, masses, kinds, domain, adjacency)
...
def test_gradientGeometryJVP_combined_matches_jacobian_reference_2d(scheme):
...
def test_gradientGeometryJVP_matches_jacobian_reference_1d(mode, scheme):
...
@pytest.mark.parametrize("scheme", list(GradientScheme))
def test_gradientGeometryJVP_matches_jacobian_reference_2d(scheme):
    positions, supports, masses = _grid_case_2d()
    domain = _make_domain(dim=2)
    n = positions.shape[0]
    kinds = torch.zeros(n, dtype=torch.int32, device=DEVICE)
    p0_forAdjacency = ParticleState(positions=positions, supports=supports, masses=masses, densities=None, kinds=kinds)
    adjacency = radiusSearchCompactHashMap(p0_forAdjacency, domain, mode=SupportScheme.Gather)
    _check_jacobian_reference(positions, supports, masses, domain, adjacency, SupportScheme.Gather, scheme)
...
def test_gradientGeometryJVP_none_adjacency_matches_explicit():
...
def test_gradientGeometryJVP_geometryOnly_unchanged_when_combination_allowed():
    # Regression guard (warpier_tier2_combined_jvp_plan.md step 5): now that
    # a geometry tangent alongside a value tangent is summed rather than
    # rejected, the geometry-only path (no value tangent at all) must still
    # return exactly what it did before -- no accidental extra value JVP term
    # sneaking in when no value tangent was supplied.
    positions, p0, domain, adjacency, props, qv, rv = _minimal_case()
...
def test_gradientGeometryJVP_rejects_missing_values():
    positions, p0, domain, adjacency, props, qv, rv = _minimal_case()
...
def test_gradientGeometryJVP_rejects_volumes():
    positions, p0, domain, adjacency, props, qv, rv = _minimal_case()
    n = positions.shape[0]
    with pytest.raises(NotImplementedError, match="geometry JVP"):
        warpOperationJVP(p0, props, domain, adjacency=adjacency,
...
def test_gradientGeometryJVP_grid_traversal_matches_adjacency_traversal():
    ...
    viaAdjacency = computeSPHGradientGeometryJVP(adjacency=adjacency, **common)
    viaGrid = computeSPHGradientGeometryJVP(adjacency=hashMap, **common)
