...
* ~~Open, and not fixable from this repo alone...~~ **Fixed — turned out to be TWO independent bugs in `wp_autograd.py`, not `warp-lang` itself, and not one bug.** Both `WarpFunctionWrapper.backward` and `StateAwareWarpFunction.backward` needed two separate, non-overlapping fixes; applying only one does not fix the other's failure mode (confirmed via a 2×2 isolation matrix, `scripts/repro_warp_grad_reentrancy.py`):
  ...
  * **Bug 2 — seeding the output gradient before `tape.backward()`.** `out_warp.grad = castTorchToWarpAsBuiltins(grad_out.contiguous())` breaks whenever a caller reuses the *same* `grad_outputs` tensor object across separate, independent `torch.autograd.grad(...)` calls (fresh forward pass and fresh `wp.Tape()` each time — a realistic pattern, e.g. a preallocated gradient-seed buffer reused every training step): correct on the first call, silently `0` on every call after. The Bug-1 fix alone does **not** fix this. Fix: force fresh storage on the incoming grad tensor (`.contiguous().detach().clone()`) before wrapping it.
  Both fixes are now applied to both `backward()` methods. Verified: `scripts/gradcheck_density_native.py` (calls `gradcheck` directly, no workarounds) passes; a reused-`grad_outputs` stress test against the real Density op (both a fresh-forward-per-call loop and repeated backward on one retained graph) passes; no regression in the forward-mode pytest suite or `operation_matrix.py --ci`. No `warp-lang` upgrade needed. Both underlying `wp.array.grad` buffer pointer identity across separate `wp.from_torch()` calls on the same tensor storage is still real and confirmed (see the repro script's docstring) — consistent with Warp keying some internal bookkeeping off memory address rather than Python object identity, but we don't have visibility into Warp's internals to say more than that.
...
* `scripts/_gradcheck_common.py` — shared, non-entrypoint helper module extracted from the two Density scripts once the pattern proved out: `DEVICE`/`DTYPE`/`KERNEL` constants, `make_domain`, `single_particle_case`, `line_case`, `build_adjacency` (frozen/non-differentiable, built once from detached positions — see its docstring for why), and the pure-PyTorch `wendland2_kernel_1d` reference kernel. Both Density scripts now import from it instead of duplicating the setup; new gradcheck scripts should too.
* `scripts/repro_warp_grad_reentrancy.py` — the minimal, sphWarpCore-independent repro underlying the two-bugs finding above, kept for reporting upstream to the `warp-lang` devs (and as a local regression guard — re-run it if `wp_autograd.py`'s `backward()` methods change). Builds a toy `y = x**2` `torch.autograd.Function` with the two candidate fixes (`write_clone`, `read_clone`+`tape_zero`) independently toggleable, and prints a 4-row × 2-column PASS/FAIL matrix (all 4 fix combinations × both bug checks) — only "both fixes" clears both columns; each fix alone clears exactly one. Confirmed on both `cpu` and `cuda`.

### Gradcheck Script Rollout Plan (Next Stages)

Stage 1 (Density) is done. Since the AD-bridge fix, **new scripts should follow `gradcheck_density_native.py`'s pattern — call `torch.autograd.gradcheck` directly** — not `gradcheck_density.py`'s manual-Jacobian workaround, which is now legacy. Each stage below is unstarted; notes are what's specific to that operator, not a full spec.

* **Stage 2 — Interpolate.** Differentiable inputs: `positions`, `supports`, `queryValues`, `referenceValues` (test both scalar- and vector-rank fields). `computeSPHInterpolation_Func` reads `queryDensities`/`referenceDensities` for the `m_j/rho_j` volume term (see `wp_interpolate.py`), so the case builder needs a precomputed `.densities` (run Density first, like the existing pytest fixtures do). Simplest first cut: treat `densities` as frozen/detached (only check the interpolate-specific derivative terms); note chaining Density's own gradient through as a stretch goal, not a requirement.
* **Stage 3 — Gradient.** This is the operator the "SPH gradient term" plot comparison was originally asked for — but that comparison is a *forward*-value check (does `WarpOperation.Gradient`'s output match a hand-coded reference SPH gradient sum for each `GradientScheme`?), distinct from the *backward*-mode gradcheck (`d(gradient_output)/d(position|value)`, effectively a second-derivative check since the forward output already depends on position through the kernel). Do both: gradcheck as usual, plus a per-particle plot of warp's forward output vs. a reference implementation, across the four `GradientScheme` variants (Naive/Symmetric/Difference/Summation — see `operation_matrix.py` for the existing set).
* **Stage 4 — Divergence.** Vector field input, scalar output. Also has `divergenceDotMode` (two conventions for matrix-field divergence — see the Reality Check entry above); worth checking both. Same `GradientScheme` variants as Gradient.
* **Stage 5 — Curl.** Vector field input; scalar output in 2D. Same `GradientScheme` variants. Worth keeping as a standing regression guard given curl's history here (the bare-`return`-in-`@wp.func` CPU/LLVM compile bug, see Reality Check).
* **Stage 6 — Laplacian.** Scalar field. Cross product of `GradientScheme` × `LaplacianScheme` (4×4, per `operation_matrix.py`) — the reference formula differs meaningfully between `Naive`/`Brookshaw`/`Dot`/`Default`, so this one needs the most care in the reference implementation.
...
* Add gradient/finite checks where applicable. **Started.** `scripts/gradcheck_density.py` covers Density (see "Backward-Mode (Reverse AD) Findings" below — this surfaced real AD-bridge bugs, not just missing coverage). Interpolate/Gradient/Divergence/Curl/Laplacian still need their own scripts.
...
* Add CI entrypoints for a deterministic subset and an extended nightly matrix. **Partially done.** `.github/workflows/tests.yml` runs on push/PR (Python 3.10-3.12, `ubuntu-latest`, CPU-only torch — GitHub-hosted runners have no CUDA so `cuda`-parametrized pytest cases self-skip and the matrix runs `--device cpu` only): the full pytest suite, then `scripts/operation_matrix.py --device cpu --ci` on the default non-jittered lattice as a hard gate (`--ci` is new: it turns the previously print-only diagnostic into a real exit-code gate, failing on any `HIGH`/`ERR`/`NAN` cell or a fatal per-device build error). An extended nightly run with `--jitter` (which actually exercises CRK/renorm, per the script's own docs) on a schedule trigger is still open, as is CUDA coverage (would need a self-hosted/GPU runner).
...
* `.github/workflows/tests.yml` — CI: pytest suite + `scripts/operation_matrix.py --ci` on CPU, push/PR triggered.
* Still missing: `tests/data/` baseline fixtures, gradient/finite-difference checks, per-test behavior-spec docs, nightly jittered/extended CI run, CUDA CI coverage.
...
* CI can block regressions before and during migration. — **Partially met.** `.github/workflows/tests.yml` blocks on pytest failures and on any non-`OK` cell in `scripts/operation_matrix.py --ci` (CPU only, non-jittered). Not met for CUDA or for the jittered/CRK-renorm-exercising configuration, which don't run in CI yet.
