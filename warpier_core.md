...
* ~~Open, and not fixable from this repo alone...~~ **Fixed — root cause was in `wp_autograd.py`, not `warp-lang` itself.** `warp-lang` 1.15.0 reuses the same `.grad` buffer for a given tensor's underlying memory address across independent `wp.from_torch()` calls (confirmed directly: identical `.grad` pointer across three separate constructions on the same tensor). `WarpFunctionWrapper.backward` (and, identically, `StateAwareWarpFunction.backward`) read that buffer straight into the returned gradient (`wp.to_torch(wa.grad)`, no copy) and never cleared it afterward — so the *next* backward call on an array sharing that same memory started from stale, non-zeroed gradient state, and the returned tensor could still be aliased to a buffer a later call would go on to mutate. Two changes to both `backward()` methods fixed it: (1) `.clone()` the gradient tensor when reading it out of Warp, so PyTorch never holds a view onto Warp-owned memory that a later call might overwrite; (2) explicitly `ctx.tape.zero()` after collecting gradients, so the underlying buffer starts clean for whatever call reuses that memory next. With both in place, `torch.autograd.gradcheck` now passes **directly** against `warpOperation` for Density (`scripts/gradcheck_density_native.py`, no workarounds) — confirmed on both the single-particle and line-of-particles cases, with no regression in the forward-mode pytest suite or `operation_matrix.py --ci`. `StateAwareWarpFunction.backward` (used by the renormalization/covariance structured-state path) got the identical fix by inspection/analogy — not yet covered by a dedicated gradcheck script, so treat that path's fix as unverified by a real test until one exists. No `warp-lang` upgrade was needed after all.
