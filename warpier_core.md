...
* `renorm/wp_renormalization.py` (new) — `computeRenormalizationMatrices_`/`computeRenormalizationMatrices`, unchanged in behavior: calls `computeCovarianceMatrix`, applies the low-neighbor-count fallback (`num_nbrs = adjacency.numNeighbors`), then `pinv2x2_warpBackend` (2D) or `torch.linalg.pinv` (3D+). **Still requires an explicit `AdjacencyList`** — the `NotImplementedError` restriction moved here unchanged, since the low-neighbor fallback reads `adjacency.numNeighbors`, which `CompactHashMap` doesn't expose (no generic per-particle neighbor count exists yet for grid traversal). Closing *this* gap — and separately, gradchecking `pinv2x2_warp` itself — is explicit follow-on work, not done here.
