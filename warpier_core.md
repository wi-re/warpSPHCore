...
**Bug found and fixed while verifying this pass, unrelated to the restructuring's own diff:** `scripts/_gradcheck_common.py` (imported by all ten `gradcheck_*.py` scripts) still did `from sphWarpCore.radiusSearch.compactHash.wp_compactHash import buildCompactHashMap` — a path that never existed under the new layout (`wp_compactHash.py`'s contents landed in `compactHash/buildHashmap.py`, not a same-named file one level deeper). This broke `tests/operations/test_gradcheck_scripts.py`'s entire parametrized suite (10/10 cases) with `ModuleNotFoundError`, even though nothing in this commit's own diff touched that script — it's the same failure class flagged after every prior restructuring pass ("grep for the old module paths after the move," per the standing rule from "Autograd Package Consolidation" above), just this time in `scripts/`, which isn't covered by the mechanical `from ..dataTypes import *`-style package re-exports that protected everything under `src/`. Fixed by importing `buildCompactHashMap` from top-level `sphWarpCore` instead (already re-exported there via `radiusSearch.__all__`), matching how the same file already imports `radiusSearchCompactHashMap`.

Validation: all 66 pytest cases pass (the 56 non-gradcheck cases were unaffected by the `_gradcheck_common.py` bug; all 10 gradcheck cases pass once that import was fixed), `from sphWarpCore import *` succeeds with zero missing `__all__` names, and `scripts/run_operation_matrix_sweep.sh --quick` is clean (`OK=258, HIGH=0, ERR=0, NAN=0, NA=42` — the 42 `NA` cells are exactly the expected `Laplacian[.../Dot]` scalar-field cases in `dim>1`, see the reclassified item below, not a regression).
...
One loose end left open, not fixed: `kernels/gradient.py`'s `sphKernelGradient` (the `xi`/`xj` variant) picked up a `/scalar_t(2.0)` in its `SuperSymmetric` branch that the pre-refactor code didn't have — but its twin `sphKernelGradient_ij` (the precomputed-`xij` variant) already had that division beforehand, so the two disagreed with each other before this change, not just with history. Ambiguous whether this is an accidental regression or an intentional correctness fix bringing the two into agreement; not exercised by the quick operation-matrix sweep or the gradcheck suite's default `SupportScheme` (`Gather`), so unverified either way. Flagged for the repo owner to confirm before relying on it.

Validation: all 67 pytest cases pass (including every `gradcheck_*_native.py` script), `scripts/run_operation_matrix_sweep.sh --quick` is clean (`OK=258, HIGH=0, ERR=0, NAN=0`), `from sphWarpCore import *` exposes every `__all__` name.
...
* **`getParticleCorrectionData_j` is unused.** Every `_Func_i` still reads `correctionData.referenceVolumes[j]`/`correctionData.referenceOmegas[j]` directly off the array-level struct (or calls `getVolume_j`/`getCRK_j` individually) rather than calling `getParticleCorrectionData_j(correctionData, j)` — so the `_j`-side bundle exists but isn't wired up, and when it is, it will eagerly build *all* of `j`'s correction terms every neighbor iteration even for operators that only need one (e.g. Density needs none, CRK-density needs only `getVolume_j`). That's some tension with the reason the individual `getVolume_j`/`getGradH_j`/`getCRK_j` accessors exist in the first place — they let a caller fetch only what it needs, per neighbor, without paying for the rest.
* **`ParticleCorrectionData_D` duplicates the four `useX` flags** (`useGradientRenormalization`/`useVolume`/`useGradHTerms`/`useCRK`) that already live, identically, as broadcast (not per-particle) values on `correctionData`. Every `_Func_i` reads them off `correctionData.useX` rather than `iCorrectionData.useX`, so the copies on the per-particle struct are currently dead weight, not wrong, just an inconsistency in which struct is the source of truth for a given field.

Neither blocks anything — all 67 pytest cases pass and `scripts/run_operation_matrix_sweep.sh --quick` stays clean (`OK=258, HIGH=0, ERR=0, NAN=0`) with this change on top of both the `kernelState` and `TrueAllToToAll` work above — but worth resolving before this pattern reaches any further files.
