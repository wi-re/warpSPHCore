...
Default-scheme Tier-2 (geometry) JVP with CRK correction enabled
(`warpier_tier2_correction_jvp_plan.md` follow-up, 2026-08-21, extending
phase (e) past its own original Brookshaw-only scope) --
`computeSPHLaplacianDefaultGeometryJVP(..., crkState=..., crkTangentState=...)`.
...
rationale -- Default's own `n_ij2 = n_ij/D2_ij` (a second, more tightly regularized
distance than Brookshaw's own `D_ij`) is the only genuinely new piece; Stages 1-4
...
(`spike_forward_mode_tier2_crk_extension.py`), one quotient-rule level deeper than Brookshaw/Dot (`n_ij2` divides by a
*second* regularized distance on top of `n_ij`'s own division). Fixed in both `wp_laplacian.py` (primal) and
`wp_laplacianJVP.py` (this Tier-2 JVP) by guarding Default's own contribution
...
every existing non-CRK gradcheck script stay bit-identical).
    ...
    python scripts/gradcheck/gradcheck_tier2_jvp_laplacian_default_crk.py
"""
...
def main():
    ...
    if ok:
        ...
        print("FAILED -- CRK tangent extension for Laplacian(Default) (warpier_tier2_correction_jvp_plan.md follow-up, 2026-08-21) has a wrong Jacobian.")
