...
backward-mode counterpart to scripts/diagnostics/operation_matrix.py, which only checks
forward values. Run as its own process:

    python scripts/gradcheck/gradcheck_density.py
    python scripts/gradcheck/gradcheck_density.py --line-n 9 --plot
...
standalone script (`python scripts/gradcheck/gradcheck_density.py`) rather than a
...
Findings" and scripts/gradcheck/gradcheck_density_native.py, which now passes calling
