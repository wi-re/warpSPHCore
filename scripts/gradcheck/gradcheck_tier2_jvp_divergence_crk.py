...
`computeSPHDivergenceGeometryJVP(..., crkState=..., crkTangentState=...)`.

Mirrors `gradcheck_tier2_jvp_gradient_crk.py`'s pattern (phase (c)) -- a much
smaller diff than that script since Stages 1-4
(`crk.computeKernelGradientCRKJVP`) are already proven and production-wired;
what's new here is just Divergence's own `dot(dcoeff,G) + dot(coeff,dG)`
combination consuming the CRK-corrected `(G, dG)` pair instead of the plain
one. Two checks:

1. **Targeted direct-tensor gradcheck** (`run_direct_gradcheck`): `crkState`/
   `crkTangentState` as independent synthetic leaf tensors, isolating
   `_jvpCommon.launchGeometryJVP`'s `flat_tensors`/`build_fn` wiring for the
   CRK tensors -- already proven generically by phase (c)'s own script since
   the wiring is shared code, but re-verified here since Divergence's own
   `computeSPHDivergenceGeometryJVP` now exposes new `crkState`/
   `crkTangentState` parameters for the first time.
2. **JVP-vs-jacobian identity** (`run_jvp_identity_case`): production's
   assembled JVP against `torch.autograd.functional.jacobian` on primal
   `warpOperation(Divergence, crkState=...)`, same pattern
   `spike_forward_mode_tier2_crk_extension.py` already validates by hand;
   this exercises it through `torch.autograd.gradcheck`'s own reverse-mode
   instead (end-to-end, `crkState`/`crkTangentState` derived from the same
   leaf positions/supports being perturbed).

    python scripts/gradcheck/gradcheck_tier2_jvp_divergence_crk.py
"""
...
from _gradcheck_common import DEVICE, DTYPE, KERNEL, build_adjacency, grid_case_2d, build_grid_adjacency
...
#    Divergence's own new crkState/crkTangentState parameters.
# ---------------------------------------------------------------------------
...
#    torch.autograd.functional.jacobian on primal warpOperation(Divergence,
#    crkState=...), through torch.autograd.gradcheck's own reverse-mode
#    (crkState/crkTangentState derived from the same leaves being perturbed).
# ---------------------------------------------------------------------------
...
def main():
    ...
    if ok:
        ...
        print("FAILED -- CRK tangent extension for Divergence (warpier_tier2_correction_jvp_plan.md phase (e)) has a wrong Jacobian.")
