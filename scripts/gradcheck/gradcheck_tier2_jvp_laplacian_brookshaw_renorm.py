...
Brookshaw-scheme Tier-2 (geometry) JVP with gradient-renormalization
correction enabled (`warpier_tier2_correction_jvp_plan.md` phase (f)) --
`computeSPHLaplacianBrookshawGeometryJVP(..., renormalizationState=..., renormalizationTangentState=...)`.
Naive/Dot/Default stay out of renorm scope (no derivation exists), matching
`operations.py`'s own restriction (same boundary as CRK, phase (e)).

See `gradcheck_tier2_jvp_divergence_renorm.py`'s docstring for the shared
rationale -- Brookshaw's own `P = dot(G,n_ij)/D_ij` weighting is the only new
piece; the `dL_i @ G + L_i @ dG` product rule (`wp_gradientJVP.py`, phase (d))
is reused verbatim through `_laplacianGeometryChainJVP`, applied after the CRK
swap (a no-op here, CRK is off). `line_case` (1D) always includes at least one
exact self-pair per query particle -- also exercises the `if r_ij > 0:` guard
phase (e)'s spike added to Brookshaw's contribution (`wp_laplacianJVP.py`),
confirming renormalization's own `matmul(L, kernelGradient)` step does not
reintroduce the self-pair adjoint bug that CRK's nonzero-at-self-pair value
did (see `spike_forward_mode_tier2_crk_extension.py`'s module docstring for
that bug's full root-cause writeup).

    python scripts/gradcheck/gradcheck_tier2_jvp_laplacian_brookshaw_renorm.py
"""
...
from warpSPHCore.coreOperations.wp_laplacianJVP import computeSPHLaplacianBrookshawGeometryJVP
...
def main():
    ...
    if ok:
        ...
        print("FAILED -- renormalization tangent extension for Laplacian(Brookshaw) (warpier_tier2_correction_jvp_plan.md phase (f)) has a wrong Jacobian.")
