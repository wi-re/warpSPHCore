...
Brookshaw-scheme Tier-2 (geometry) JVP with CRK correction enabled
(`warpier_tier2_correction_jvp_plan.md` phase (e)) --
`computeSPHLaplacianBrookshawGeometryJVP(..., crkState=..., crkTangentState=...)`.
Naive/Dot/Default stay out of CRK scope (no derivation exists), matching
`operations.py`'s own restriction.
...
piece; Stages 1-4 (`crk.computeKernelGradientCRKJVP`) are reused verbatim
from phase (c).

**Found and fixed a genuine, pre-existing reverse-mode adjoint bug while
writing this script, not introduced by this phase**: see
`spike_forward_mode_tier2_crk_extension.py`'s module docstring for the full
root-cause writeup (an exact self-pair, `r_ij == 0`, combined with CRK's
generically-nonzero `correctGradientCRK` value at the kernel's own peak, and
Brookshaw's *second* division of `dot(kernelGradient, n_ij)` by `D_ij` on top
of `n_ij`'s own `x_ij/D_ij` division). Fixed in both `wp_laplacian.py`
(primal) and `wp_laplacianJVP.py` (this Tier-2 JVP) by guarding the
Brookshaw contribution with an explicit `if r_ij > 0:` -- the true
contribution at `r_ij == 0` is always exactly `0` regardless of CRK, so this
changes no forward value anywhere (confirmed: `operation_matrix.py` and
every existing non-CRK gradcheck script stay bit-identical).

    python scripts/gradcheck/gradcheck_tier2_jvp_laplacian_brookshaw_crk.py
"""
...
def main():
    ...
    if ok:
        ...
        print("FAILED -- CRK tangent extension for Laplacian(Brookshaw) (warpier_tier2_correction_jvp_plan.md phase (e)) has a wrong Jacobian.")
