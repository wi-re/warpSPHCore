...
Dot-scheme Tier-2 (geometry) JVP with gradient-renormalization correction
...
`computeSPHLaplacianDotGeometryJVP(..., renormalizationState=..., renormalizationTangentState=...)`.
...
rationale -- Dot's own `F_ab = dot(G,n_ij)/D_ij` weighting is the only
reused piece; the `dL_i @ G + L_i @ dG` product rule (`wp_gradientJVP.py`,
...
the `if r_ij > 0:` guard this follow-up's own spike added to Dot's
...
docstring for that bug's full root-cause writeup).
    ...
    python scripts/gradcheck/gradcheck_tier2_jvp_laplacian_dot_renorm.py
"""
...
def main():
    ...
    if ok:
        ...
        print("FAILED -- renormalization tangent extension for Laplacian(Dot) (warpier_tier2_correction_jvp_plan.md follow-up, 2026-08-21) has a wrong Jacobian.")
