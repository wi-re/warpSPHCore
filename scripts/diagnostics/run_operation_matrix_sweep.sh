#!/usr/bin/env bash
#
# Wrapper around scripts/diagnostics/operation_matrix.py for two common invocation
# patterns:
#
...
#
# Usage:
#   scripts/diagnostics/run_operation_matrix_sweep.sh
...
run() {
  ...
  python scripts/diagnostics/operation_matrix.py "$@"
}
...
python scripts/diagnostics/operation_matrix.py --device cpu --jitter 0.3 --verbose || true
